...
import { getAffiliateMetrics } from "../lib/supabaseAdmin";
...
const Affiliate = () => {
  ...
  useEffect(() => {
    ...
    const loadMetrics = async () => {
      ...
      try {
        const data = await getAffiliateMetrics();
        if (cancelled) return;
        ...
        setMetrics(null);
        setError("Unable to load affiliate metrics right now.");
      } finally {
        if (!cancelled) {
          setLoading(false);
        }
  ...
  const totalEligible = metrics?.totalEligible ?? 0;
